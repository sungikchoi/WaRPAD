...
parser.add_argument('--prep_size', default=224, type =int)
...
dino_transform = transforms.Compose([transforms.Resize(args.prep_size,interpolation=Image.BICUBIC), transforms.CenterCrop((args.prep_size,args.prep_size)), transforms.ToTensor(), normalize])
...
def cal_metric(model, input_tensor):
    ...
    with torch.no_grad():
        ...
        perturbed_tensor = input_tensor + args.noise_level * torch.randn_like(input_tensor)
...
folder_name = './paper_stats/genimage/rigid_cc/'+str(args.prep_size)+'/'+str(args.patch_size)+'/' + str(args.noise_level) +'/'
